...
class NLIDataloader():
    ...
    def load_nlidata(self, batch_size, gpu_option):
        ...
        LABEL_FIELD = torchtext.data.Field(sequential=False, batch_first=True)
    ...
    def load_mutidata_json(self, text_field, label_field):
        ...
        train, match, mis_match = torchtext.data.TabularDataset.splits(path=self.multinli_path, format='json', # TODO
    ...
    def load_snlidata_json(self, text_field, label_field):
        return torchtext.datasets.SNLI.splits(text_field=text_field, label_field=label_field, root=self.snli_path)
